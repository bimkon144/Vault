...
async function main() {
  ...
  console.log("balanceOfToken0:", token0.address);
  const Vault = await hre.ethers.getContractFactory("Vault");
  const vault = await Vault.deploy("shareToken", "STK", token0.address);


  await token0.approve(vault.address, 500);
  await vault.deposit1(500, owner.address);
  const assets = await vault.totalSupply();
  console.log('supply', assets);

  // const balanceOfToken1 = await token0.balanceOf(vault.address);
  // console.log(balanceOfToken1);

  // const balanceOfToken3 = await vault.balanceOf(owner.address);
  // console.log(balanceOfToken3);
